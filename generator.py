...
#         creation = ["DROP DATABASE grs IF EXISTS","CREATE database grs "]
...
def read_csv(csv_file_name):
    ...
    skills = list(zip(skill_columns, [i for i in range(len(skill_columns))]))
    ...
    for int_id in interest_ids.keys():
        user_interest[int_id] = user_interest['Project Type Interest (Check all that apply or none)'].map(lambda x: int_id in x)
    ...
    team_preference['user_prefers'] = team_preference['value'].map(lambda x: user_email[x] if x in user_email.keys() else -1)
    ...
    return user, skills, skillset, availability, interest_ids, user_interest, team_preference, team_names_to_ids, teams
